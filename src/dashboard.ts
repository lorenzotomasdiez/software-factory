...
function summarizeSessions(events: SfEvent[]): SessionSummary[] {
  const bySession = new Map<string, SfEvent[]>();
  for (const e of events) {
    if (!bySession.has(e.session_id)) bySession.set(e.session_id, []);
    bySession.get(e.session_id)!.push(e);
  }
  const summaries: SessionSummary[] = [];
  for (const [session_id, evts] of bySession) {
    const first = evts[0];
    ...
    summaries.push({
      ...
      status: evts.some((e) => e.type === "session_end") ? "ended" : "running",
  ...
  Bun.serve({
    ...
    fetch(req) {
      ...
      if (url.pathname === "/api/events") {
        ...
        return Response.json(session ? events.filter((e) => e.session_id === session) : events);
      }
